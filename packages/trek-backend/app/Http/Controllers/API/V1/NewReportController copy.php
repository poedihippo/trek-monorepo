...
class NewReportControllerCopy extends BaseApiController
{
        ...
        if (in_array($userType, ['director', 'hs'])) {
            if ($request->user_type == 'bum') {
                $query = User::selectRaw('id, name')
                    ...
                    ->where('supervisor_type_id', 2)
                    ->where('company_id', $companyId)
                    ->with(['channels' => function ($channel) use ($companyId, $channelId, $startDate, $endDate, $startDateCompare, $endDateCompare) {
                ...
                $query = Channel::where('company_id', $companyId)
                    ->with(['sales' => function ($q) use ($channelId, $startDate, $endDate, $startDateCompare, $endDateCompare) {
                        $q->withCount(['leads as total_leads' => function ($q) use ($startDate, $endDate) {
                            $q->whereCreatedAtRange($startDate, $endDate);
                ...
                foreach ($result as $channel) {
                    ...
                    $sales_compare_quotation = 0;
                    $sales_deals = 0;
                    $saels_total_deals_transaction = 0;
                    $sales_compare_deals = 0;
                    $sales_interior_design = 0;
                    ...
                    foreach ($channel->sales as $sales) {
                        ...
                        $sales_compare_quotation += $sales->compare_total_quotation ?? 0;
                        $sales_deals += $sales->total_deals ?? 0;
                        $saels_total_deals_transaction += $sales->total_deals_transaction ?? 0;
                        $sales_compare_deals += $sales->compare_total_deals ?? 0;
                        $sales_interior_design += $sales->interior_design ?? 0;
                    ...
                    $data['details'][] = [
                        ...
                        'deals' => [
                            ...
                            'total_transaction' => $saels_total_deals_transaction,
                        ],
                        'interior_design' => [
                        ...
                        'retail' => [
                            ...
                            'total_transaction' => $saels_total_deals_transaction - $saels_interior_design_total_transaction,
                        ],
                    ];
                    ...
                    $summary_total_deals_transaction += $saels_total_deals_transaction ?? 0;
            ...
            if ($request->user_type == 'store') {
                $query = Channel::whereIn('id', $user->channels->pluck('id')->all())
                    ->with(['sales' => function ($q) use ($channelId, $startDate, $endDate, $startDateCompare, $endDateCompare) {
                        $q->withCount(['leads as total_leads' => function ($q) use ($startDate, $endDate) {
                            $q->whereCreatedAtRange($startDate, $endDate);
            ...
            $query = Channel::with(['sales' => function ($q) use ($channelId, $startDate, $endDate, $startDateCompare, $endDateCompare) {
                $q
                    ->withCount(['leads as total_leads' => function ($q) use ($startDate, $endDate) {
                        $q->whereCreatedAtRange($startDate, $endDate);
