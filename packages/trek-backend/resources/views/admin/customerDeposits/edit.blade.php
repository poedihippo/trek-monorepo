...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.payments.update", [$customerDeposit->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number"
                           name="amount" id="amount" value="{{ old('amount', $customerDeposit->amount) }}" step="0.01" required>
                ...
                @livewire('payment-page', ['payment_type' => old('payment_type_id', $customerDeposit->payment_type_id)])
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('reference') ? 'is-invalid' : '' }}" type="text"
                           name="reference" id="reference" value="{{ old('reference', $customerDeposit->reference) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                            ...
                            required>
                        <option value
                                disabled {{ old('status', $customerDeposit->status) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach(\App\Enums\PaymentStatus::getInstances() as $enum)
                            <option value="{{ $enum->value }}" {{ old('status', $customerDeposit->status->value) == $enum->value ? 'selected' : '' }}>{{ $enum->description }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text"
                           name="reason"
                           id="reason" value="{{ old('reason', $customerDeposit->reason) }}">
...
@section('scripts')
    <script>
        ...
        Dropzone.options.proofDropzone = {
            ...
            init: function () {
                @if(isset($customerDeposit) && $customerDeposit->proof)

                var files =
                {!! json_encode($customerDeposit->proof) !!}
