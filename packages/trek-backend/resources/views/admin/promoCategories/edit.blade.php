...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.promo-categories.update", [$promoCategory->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <x-input key='name' value="{{$promoCategory->name}}" :model='app(\App\Models\Promo::class)'></x-input>
                <div class="form-group">
                    <label for="description">{{ trans('cruds.promoCategory.fields.description') }}</label>
                    <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}"
                              name="description" id="description">{{$promoCategory->description}}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($companies as $id => $company)
                            <option value="{{ $id }}" {{ $promoCategory->company_id == $id ? 'selected' : '' }}>{{ $company }}</option>
                        @endforeach
                    </select>
...
@section('scripts')
                            ...
                            return loader.file
                                .then(function (file) {
                                    return new Promise(function (resolve, reject) {
                                        ...
                                        data.append('crud_id', '{{ $promoCategory->id ?? 0 }}');
                                        xhr.send(data);
                                    });
                                })
                        }
    ...
    <script>
        ...
        Dropzone.options.imageDropzone = {
            ...
            init: function () {
                @if(isset($promoCategory) && $promoCategory->image)
                var files =
                {!! json_encode($promoCategory->image) !!}
                    for (var i in files) {
                    var file = files[i]
