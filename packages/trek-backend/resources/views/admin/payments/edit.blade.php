...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data" class="form-loading">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number"
                           name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" step="0.01" required>
                ...
                @livewire('payment-page', ['payment_type' => old('payment_type_id', $payment->payment_type_id)])
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('reference') ? 'is-invalid' : '' }}" type="text"
                           name="reference" id="reference" value="{{ old('reference', $payment->reference) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                            ...
                            required>
                        <option value
                                disabled {{ old('status', $payment->status) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach(\App\Enums\PaymentStatus::getInstances() as $enum)
                            <option value="{{ $enum->value }}" {{ old('status', $payment->status->value) == $enum->value ? 'selected' : '' }}>{{ $enum->description }}</option>
                        @endforeach
                    </select>
                ...
                @if ($payment->status->is(\App\Enums\PaymentStatus::PENDING))
                <div class="form-group">
                    <label for="created_at">Uploaded Date</label>
                    <input type="text" name="created_at" value="{{ $payment->created_at }}" class="form-control datetime {{ $errors->has('created_at') ? 'is-invalid' : '' }}" required>
                ...
                {{-- <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text"
                           name="reason"
                           id="reason" value="{{ old('reason', $payment->reason) }}">
