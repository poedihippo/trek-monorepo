    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="form-group">
                <label>{{ trans('cruds.customer.fields.title') }}{{$customer->title->value}}</label>
                <select class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" name="title" id="title">
                    ...
                    @foreach(\App\Enums\PersonTitle::getInstances() as $enum)
                        <option value="{{ $enum->value }}" {{ old('title', $customer->title->value) == $enum->value ? 'selected' : '' }}>{{ $enum->description }}</option>
                    @endforeach
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description"
                            id="description">{{ old('description', $customer->description) }}</textarea>
            ...
            @if (count($addresses) > 0)
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('default_address_id') ? 'is-invalid' : '' }}"
                            name="default_address_id" id="default_address_id">
                        @foreach($addresses as $id => $address)
                            <option value="{{ $id }}" {{ old('default_address_id', $customer->default_address_id) == $id ? 'selected' : '' }}>{{ $address }}</option>
                        @endforeach
                    </select>
