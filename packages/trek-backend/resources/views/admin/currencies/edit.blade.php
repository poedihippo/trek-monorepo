    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.currencies.update", $currency->id) }}">
            ...
            <div class="form-group">
                ...
                {{-- <select name="main_currency" id="main_currency" class="form-control select2" required>
                    @foreach ($currencyList as $key => $value)
                        <option value="{{$key}}" {{ $currency->main_currency == $key ? 'selected' : '' }}>{{$value}}</option>
                    @endforeach
                </select> --}}
            ...
            <div class="form-group">
                ...
                <select name="foreign_currency" id="foreign_currency" class="form-control select2" required>
                    @foreach ($currencyList as $key => $description)
                        <option value="{{$key}}" {{ $currency->foreign_currency == $key ? 'selected' : '' }}>{{$description}}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="number" name="value" id="value" value="{{ $currency->value }}" required>
                @if($errors->has('value'))
