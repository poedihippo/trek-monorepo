    ...
    return axios
      .get(
        `targets?filter[type]=DEALS_INVOICE_PRICE&filter[start_after]=${moment(
          ...
          .format(
            "YYYY-MM-DD",
          )}&filter[reportable_type]=USER&filter[company_id]=&filter[supervisor_type_level]=1&filter[descendant_of]=${
          params?.id
        }`,
      )
      .then((res) => {
    ...
    return (
      <>
        ...
        >
          ...
          >
            <Div row>
              <Div style={{ width: widthPercentageToDP(60) }}>
...
export default RevenueStoreLeader
