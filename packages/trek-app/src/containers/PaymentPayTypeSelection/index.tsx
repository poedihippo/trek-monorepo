...
import usePaymentTypeList from "api/hooks/payment/usePaymentTypeList"
...
type CurrentScreenRouteProp = RouteProp<
  CustomerStackParamList,
  "PaymentPayTypeSelection"
>
type CurrentScreenNavigationProp = CompositeNavigationProp<
  CompositeNavigationProp<
    StackNavigationProp<CustomerStackParamList, "PaymentPayTypeSelection">,
...
export default () => {
  ...
  const paymentCategoryId = route?.params?.paymentCategoryId ?? -1
  if (orderId === -1 || paymentCategoryId === -1) {
  ...
  const {
    queries: [{ data: paymentTypePaginatedData }],
    meta: {
      isError,
      isLoading,
      isFetching,
    ...
    usePaymentTypeList({
      filterPaymentCategoryId: paymentCategoryId.toString(),
    }),
  ] as const)

  const data: PaymentType[] = dataFromPaginated(paymentTypePaginatedData)
  ...
  return (
    <FlatList
      ...
      data={data}
      keyExtractor={({ id }) => `paymentType${id}`}
      ...
      renderItem={({ item, index }) => (
        <Pressable
          onPress={() =>
            navigation.navigate("PaymentPayConfirm", {
              orderId,
